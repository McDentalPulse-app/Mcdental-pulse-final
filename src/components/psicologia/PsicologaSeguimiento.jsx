...
const PsicologaSeguimiento = ({ encuestas, notas, onUpdateNota, onDeleteNota }) => {
  ...
  // Sin encuesta contestada no es "verde" — es que no hay dato (mismo fix que en
  // /empleados, antes se veía igual que alguien de verdad estable).
  const getUltimoSemaforo = (empId) => {
    const enc = encuestas.filter(e => e.empleadoId === empId).sort((a, b) => b.semana.localeCompare(a.semana));
    return String(enc[0]?.semaforo || "sin-datos").toLowerCase();
  };
