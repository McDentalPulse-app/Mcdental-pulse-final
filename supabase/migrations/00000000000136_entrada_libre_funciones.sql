-- ============================================================================
-- Entrada libre (sin geocerca, sin retardo) — migración 136. Generaliza el mismo
-- bloque de "libre" que ya existe para salida (migración 127) y agrega el ajuste de
-- hora en registrar_checada. p_entrada_libre es lo que pide el CLIENTE en esa checada
-- puntual (el interruptor del Checador); las tres funciones vuelven a leer
-- puede_marcar_entrada_libre de la base antes de honrarlo — nunca se confía el flag
-- solo, igual que ya hace puede_marcar_salida_sin_geocerca.
-- ============================================================================

-- CRITICO: agregar un parámetro con `create or replace` NO reemplaza la función — Postgres
-- la trata como un overload nuevo porque la firma (lista de tipos) cambió, y deja la vieja
-- viva al lado. Cualquier llamada que mande solo los argumentos de antes (exactamente lo que
-- hacían api/checar.js y las propias funciones entre sí) queda ambigua entre las dos y
-- Postgres la rechaza — habría tumbado CADA checada de la app en cuanto se aplicara esto.
-- Se borran las firmas anteriores primero, así solo queda una versión de cada función.
drop function if exists public.sucursal_para_checada(uuid, numeric, numeric, integer);
drop function if exists public.sucursal_para_checada(uuid, numeric, numeric, integer, public.tipo_checada);
drop function if exists public.checar_ubicacion(uuid, numeric, numeric, integer);
drop function if exists public.checar_ubicacion(uuid, numeric, numeric, integer, public.tipo_checada);
drop function if exists public.registrar_checada(uuid, public.tipo_checada, numeric, numeric, integer, text, text);
...
begin
  ...
  select u.sucursal, coalesce(u.puede_marcar_en_cualquier_clinica, false),
         ...
         coalesce(u.puede_marcar_entrada_libre, false)
    into v_nombre_suc, v_permiso, v_salida_libre, v_entrada_libre
  ...
  -- Entrada libre: igual, pero además exige que ESTA checada la haya pedido con el
  -- interruptor (p_entrada_libre) — el permiso solo no basta, a diferencia de salida.
  if p_tipo = 'entrada' and v_entrada_libre and p_entrada_libre then
    estado := 'dentro'::public.estado_ubicacion;
    distancia := null;
    return;
  end if;
