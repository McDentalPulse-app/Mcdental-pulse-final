-- El sábado, si el empleado no contestó su encuesta semanal de pulso, no puede marcar
-- SALIDA hasta que la conteste (la entrada no se toca). Misma filosofía que la geocerca
-- o el cotejo facial: el guard vive en registrar_checada porque es la ÚNICA función que
-- puede escribir una checada (revocada de authenticated) — ponerlo solo en api/checar.js
-- dejaría la regla evadible para quien llame la RPC directo.
--
-- to_char(fecha, 'IYYY-"W"IW') reproduce el mismo formato "YYYY-Www" que getISOWeek()
-- (src/utils/constants.js): mismo estándar ISO-8601 (semana empieza lunes, año ISO).
