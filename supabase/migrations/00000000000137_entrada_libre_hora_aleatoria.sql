-- Entrada libre: la hora que se guarda deja de ser SIEMPRE la de inicio de turno exacta
-- (10:00:00.000 clavado todos los días se ve raro en cualquier reporte) y pasa a ser
-- aleatoria dentro de los 30 minutos antes del turno — pedido del dueño: "9:31, 9:32,
-- 9:45, etc". Mismo signature, CREATE OR REPLACE de verdad (no agrega parámetros, no
-- crea overload — el problema de la migración 136 no aplica aquí).
...
-- ----------------------------------------------------------------------------
-- VERIFICACIÓN: probado en transacción con rollback contra la base real — la hora que
-- sale cae siempre entre (hora_entrada - 30 min) y hora_entrada, nunca justo la exacta
-- repetida en cada prueba.
-- ----------------------------------------------------------------------------
