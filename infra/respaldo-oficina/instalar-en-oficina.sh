# ─────────────────────────────────────────────────────────────────────────────
# Instalacion del respaldo externo de Pulse en la maquina de la oficina.
# Se ejecuta UNA VEZ. No necesita sudo salvo que falte pg_restore o rsync.
# ─────────────────────────────────────────────────────────────────────────────
mkdir -p ~/pulse-respaldo/copias && cd ~/pulse-respaldo

# Huella del servidor, fijada de antemano: asi esta maquina no tiene que
# "confiar a ciegas" la primera vez que se conecta.
cat > servidor_conocido <<'FIN_HOST'
2.25.150.106 ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIMcI1Tx8VacNsgYq7H6VeN7FYWRLrzJFsBzAbKpN675l
FIN_HOST

# Secreto con el que esta maquina le cuenta a Pulse como fue el respaldo.
printf '__NO_SE_VERSIONA__' > secreto
chmod 600 secreto

# El script que copia y verifica.
cat > tirar-respaldo.sh <<'FIN_SCRIPT'
#!/bin/sh
...
FIN_SCRIPT
chmod 700 tirar-respaldo.sh

# Clave dedicada SOLO a esto. Se genera aqui: la privada nunca sale de esta maquina.
[ -f clave ] || ssh-keygen -t ed25519 -N '' -C "respaldo-pulse-oficina" -f clave -q
chmod 600 clave

# Cron diario a las 03:20 de ESTA maquina (el servidor genera el suyo a las 02:43).
(crontab -l 2>/dev/null | grep -v pulse-respaldo; echo "20 3 * * * $HOME/pulse-respaldo/tirar-respaldo.sh") | crontab -

echo
echo "================ DIAGNOSTICO ================"
echo "usuario     : $(whoami)@$(hostname)"
echo "hora local  : $(date '+%F %H:%M %Z')"
echo "rsync       : $(command -v rsync      || echo 'FALTA -> sudo apt install -y rsync')"
echo "pg_restore  : $(command -v pg_restore || echo 'FALTA -> sudo apt install -y postgresql-client')"
echo "espacio     : $(df -h "$HOME" | tail -1 | awk '{print $4" libres"}')"
echo "cron        : $(crontab -l 2>/dev/null | grep -c pulse-respaldo) linea(s) instalada(s)"
echo
echo "========= PASALE ESTA LINEA A CLAUDE ========="
cat clave.pub
echo "=============================================="
