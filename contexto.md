...
## 🚀 Sesión 2026-07-24/25 — Calendario de asistencia, bloqueo de salida por encuesta, y migración completa a VPS propia

Sesión operada por **Claude (otra máquina, vía SSH a `helminth@100.92.81.83`)**, continuando el trabajo que había quedado a medias de la sesión anterior (3 tareas: medallas ✅, vacaciones/permisos ✅, **calendario grande de asistencia** — la que faltaba).

### 1. Terminado: calendario grande por empleado (Asistencia)
El refactor de `AsistenciaPanel.jsx` (lista de empleados + calendario grande por seleccionado) ya estaba escrito por la sesión anterior pero **sin una sola línea de CSS** — se veía sin estilo. Se agregó el bloque CSS completo en `App.css` (`asistencia-layout`, `asistencia-emp-lista`, `asistencia-cal-*`, `asistencia-leyenda`, `asistencia-calendario--grande`), reusando el patrón visual de `.mensajes-layout` y `.cal-*` ya existentes. Se quitó un import muerto (`SectionTitle`). Verificado con 322/322 tests, build verde, y probado en vivo contra el dev server (`nohup npm run dev -- --host 0.0.0.0 --port 5173`, seguía corriendo desde la sesión anterior).

### 2. Feature nueva: bloquear salida el sábado si no contestó la encuesta semanal
Pedido del usuario, con interview de alcance antes de tocar código: aplica a empleado+doctor (únicos roles que checan), usa la semana ISO actual (`semanaActual`), UX de mensaje+botón a la encuesta (mismo patrón que la puerta de "rostro no registrado").
- **Migración 082** (`bloqueo_salida_sin_encuesta.sql`): `registrar_checada` — nuevo guard, sábado (`isodow=6`) + sin fila en `encuestas` para `to_char(fecha,'IYYY-"W"IW')` → `RAISE EXCEPTION`. Vive en la RPC (no en `api/checar.js`) por el mismo motivo que la geocerca: es la única puerta real, revocada de `authenticated`.
- Cliente: `ChecadorEmpleado.jsx` (+prop `encuestas`, cálculo `encuestaPendiente`), `EmpleadoLayout.jsx`/`DoctorLayout.jsx` pasando el prop.
- Verificado con SQL aislado en transacción con rollback (sábado sin encuesta bloquea, con encuesta no bloquea, entre semana nunca bloquea) — no se pudo probar en vivo porque todas las cuentas de prueba locales están sin rostro registrado y ese día no era sábado.

### 3. Excel de contraseñas sin cambiar
A pedido del usuario: reporte de los 82 empleados/doctores activos en prod con `debe_cambiar_password = true`, exportado a `~/Documentos/Empleados_contrasena_default_2026-07-24.xlsx` (máquina local del usuario, no en este repo).

### 4. Deploy a producción (código + 2 migraciones)
El usuario pidió subir todo. Antes de tocar prod: revisión completa del diff pendiente (incluida la migración 081 y ~25 archivos de la sesión anterior nunca commiteados — sucursales dinámicas, medallas, IdeasMejora, eliminar archivo de expediente, rediseño de permisos) vía subagente `code-reviewer` independiente → aprobado sin bloqueantes.
- `033f0db` — todo lo anterior + migración 081.
- `29bc64e` — bloqueo de salida por encuesta + migración 082.
- Push a `origin` y `prod` → deploy automático de Vercel confirmado (Ready, producción).
- Migraciones 081 y 082 aplicadas a **Supabase Cloud** vía Management API + PAT (mismo método de siempre), ledger 080→082 verificado.

### 5. Migración completa a una VPS propia (self-hosted, independiente de Supabase/Vercel)
Pedido grande: el usuario quiere sacar Pulse de Vercel+Supabase Cloud a una VPS de Hostinger propia (KVM8: 8 CPU/32GB/400GB, IP pública `2.25.150.106`), con **independencia total** (nada de servicios pagos externos), pero **self-host del stack open source de Supabase** (no reescritura de RLS/Auth/PostgREST — eso sería meses de riesgo para nada, se lo expliqué y el usuario confirmó que quería A, no B). Se armó un plan completo (`/home/alfredo/.claude/plans/mighty-watching-phoenix.md` en la máquina donde corrió Claude) y se ejecutó de punta a punta:

**Hallazgo de arranque**: la VPS ya tenía un stack Supabase self-hosted corriendo para otro proyecto del usuario ("Dentra", en `/opt/dentra/dentra-supabase`, toolkit oficial `supabase/docker` con wrapper `run.sh/setup.sh/reset.sh`). Pulse se instaló en un stack **100% separado** en `/opt/pulse/pulse-supabase/supabase-project/`.

**⚠️ Incidente (resuelto)**: el `docker-compose.yml` oficial trae `name: supabase` fijo — dos instancias con ese mismo nombre de proyecto Docker se pisan entre sí (Docker las trata como EL MISMO proyecto sin importar el directorio ni el `container_name`). Tumbó los contenedores de Dentra dos veces antes de fijar `COMPOSE_PROJECT_NAME=pulse` explícito en el `.env`. Dentra se restauró las dos veces sin pérdida de datos (bind mount en ruta separada, nunca tocado). Detalle completo + el fix + una segunda trampa relacionada (Kong no resolvía Realtime tras el rename, por un hostname legacy hardcodeado en `kong.yml`) quedaron documentados en la memoria de Claude (`~/.claude/projects/-home-alfredo/memory/vps_supabase_selfhost_project_name.md` en la máquina del usuario), para el próximo proyecto que se agregue a esta VPS.

**Puertos self-hosted de Pulse** (127.0.0.1, igual patrón de seguridad que Dentra — nada escucha en la interfaz pública): Postgres 5433, Pooler 6544, Kong HTTP 8010, Kong HTTPS 8453.

**Migración de esquema+datos**: en vez de solo correr las 82 migraciones locales (que no capturan la deriva de esquema ya documentada arriba — "GRANTs faltantes", "policies aplicadas a mano"), se usó `supabase db dump --linked` (CLI de Supabase, autenticado con el PAT vía `SUPABASE_ACCESS_TOKEN`) para volcar el **esquema real de prod** (public+auth, sin tocar `storage` que lo gestiona el propio contenedor `storage-api`) y los **datos reales** (`--data-only`, aplicados con `session_replication_role=replica` para saltar el FK circular de `usuarios` sin tener que calcular orden de inserción a mano). Verificado con conteos exactos: usuarios 102/102, rostros 15/15, encuestas 52/52, `auth.users` 102/102, `asistencias` 79 vs 83 (diferencia normal, 4 checadas reales pasaron en prod mientras corría el dump). **El hash de contraseña de `auth.users` es idéntico byte a byte (MD5 comparado) al de prod** — nadie tiene que resetear su contraseña.

**Storage**: 197/197 objetos (buckets `asistencias`/`rostros`/`avatars`/`expedientes`/`comisiones`) migrados vía script Node (`fetch` nativo) descargando de Cloud Storage API (con el `service_role` real, sacado del Dashboard porque en Vercel está marcada "Sensitive" y `vercel env pull` la trae vacía) y resubiendo al storage self-hosted.

**Empaquetado Docker** (nuevo, en la raíz del repo — **sin commitear todavía**, están solo en `/opt/pulse/app` en la VPS, copiados por tar, no por git):
- `Dockerfile.frontend` — build de Vite (`node:20-bookworm-slim` → `nginx:1.27-alpine`), con `VITE_SUPABASE_URL=https://mcdentalpulse.duckdns.org` y el `ANON_KEY` propio de Pulse (no el de Cloud) horneados en el bundle.
- `Dockerfile.api` + `server.js` — wrapper Express nuevo que monta cada `api/*.js` como ruta (`app.all("/api/<nombre>", handler)`), sin reescribir la lógica de negocio. `node:20-bookworm-slim` (no alpine, por los binarios nativos de `onnxruntime-node`/`sharp`). Se agregó `express` como dependencia real (`npm install express --save`).
- Los 2 crons de `vercel.json` (`limpiar-fotos` 4:17am, `tareas-programadas` 1:00pm) replicados con `crontab` del sistema + un `CRON_SECRET` propio generado (`openssl rand -hex 24`, en `/opt/pulse/.cron_secret`), llamando al `api-server` con `Authorization: Bearer`.

**Red pública**: subdominio gratis `mcdentalpulse.duckdns.org` (DuckDNS, cuenta del usuario) apuntando por A record directo a la IP de la VPS — sin túnel, la VPS ya es pública. `certbot --nginx` para TLS (Let's Encrypt, autorenovación configurada). Nginx del sistema rutea por `location`: `/auth|rest|storage|realtime|functions|graphql|pg` → Kong (`127.0.0.1:8010`), `/api/` → api-server (`127.0.0.1:3001`), `/` → frontend (`127.0.0.1:3080`).

**Verificado en vivo** (público, HTTPS): frontend 200, RLS bloqueando anónimos (`usuarios` da `[]` sin JWT), Auth procesando login contra los datos reales migrados, **Realtime WebSocket dando `101 Switching Protocols`** (tras el fix del alias de red de arriba). Lo único que no se pudo probar (ni se debía) fue un login real de punta a punta — eso lo tiene que confirmar el usuario con su propia contraseña.

### ⏳ Estado al cierre — deploy en PARALELO, no hay corte todavía
- **Producción real sigue siendo Vercel + Supabase Cloud**, sin cambios, sin downtime, nadie del equipo lo notó.
- `https://mcdentalpulse.duckdns.org` es una **copia completa y funcional** corriendo en la VPS, con los mismos datos reales (congelados al momento del dump, 2026-07-25).
- **Pendiente, fuera de esta sesión** (requiere autorización aparte): que el usuario confirme login real en la copia de la VPS; recién ahí — apagar Vercel, mover el dominio/URL real de los empleados a la VPS, decidir qué hacer con el proyecto de Supabase Cloud (dejarlo unos días de rollback antes de tocarlo).
- Los archivos Docker (`Dockerfile.*`, `server.js`, `nginx.frontend.conf`) están en el repo local de `helminth` pero **sin commitear** — decidir si se comitean tal cual o se ajustan primero.
- El `.cron_secret` y las llaves generadas por `setup.sh` para Pulse (JWT_SECRET, ANON_KEY, SERVICE_ROLE_KEY propios, distintos a los de Cloud) viven únicamente en `/opt/pulse/pulse-supabase/supabase-project/.env` de la VPS — no están en ningún repo ni chat, respaldar ese archivo aparte si se pierde la VPS.

---

## 🎨 Sesión 2026-07-25 (parte 2) — Rediseño de tablas y gestión de personal, SOLO en Docker

**⚠️ Regla de esta sesión, pedida explícitamente por el usuario: "los cambios los haremos solo en docker".**
Todo lo de abajo se editó **directo en `/opt/pulse/app` de la VPS**, NO en este repo. Desde
esta sesión **`/opt/pulse/app` y este repo dejaron de ser la misma fuente de verdad**: llevar
esto a producción (Vercel) sería un diff manual, no un merge.

**Runbook operativo completo (cómo entrar a la VPS, desplegar, dónde están las llaves,
trampas conocidas): `/opt/pulse/HANDOFF.md` en la VPS, con copia en
`helminth:/home/helminth/HANDOFF-pulse-vps.md`.**

### Tabla estilo Untitled UI en /empleados y /usuarios
Las dos pantallas pasaron de tarjetas en grilla a tabla: avatar + nombre, encabezados
ordenables por columna, paginación client-side (12 filas) y acciones como botones de ícono.
Se recreó el look con los componentes propios del proyecto (el kit `@untitledui/react` es
pago y no está instalado; solo están los iconos). `admin/usuarios` y `admin|rh|psicologa/empleados`
comparten componente, así que los tres roles quedaron cubiertos de una.
- Nuevo `common/SortableTh.jsx` compartido (antes el encabezado ordenable vivía suelto dentro
  de `EmpleadosList`).
- El detalle de empleado pasó de reemplazar la pantalla a un **panel deslizante** desde la
  derecha (overlay + Escape + clic fuera), con la tabla intacta detrás.

### 🐛 Bugs reales encontrados al probar en vivo (no eran cosméticos)
1. **"Sin evaluar" vs "Estable"**: `getUltimoSemaforo` caía a `"verde"` cuando el empleado no
   tenía NINGUNA encuesta — 82 personas sin datos se veían como "Estable". Ahora es un estado
   propio `sin-datos` ("Sin evaluar", gris). Mismo bug estaba en `PsicologaSeguimiento.jsx`.
2. **El slideout quedaba debajo del navbar**: subir el `z-index` no alcanzó — un ancestro
   (`.app-main`) crea un *stacking context* que atrapa al `position: fixed`. Se resolvió con
   un **portal a `document.body`** (`createPortal`), que es inmune a eso.
3. **Layout apretado dentro del panel**: los media queries que apilan el detalle miran el
   *viewport*, no el ancho del panel, así que en monitor grande se aplicaba el layout de 2
   columnas dentro de un panel de 640px. Se fuerza una sola columna dentro de `.mc-slideout-panel`.
4. **`.mc-form-hint--warn` es `display:flex`** → el aviso de contraseña temporal salía partido
   en 3 columnas. Mismo tropiezo ya documentado con `.mc-hint` en `MiRostro.jsx`; mismo arreglo
   (envolver el texto en un `<span>`).
5. **Las 3 Edge Functions reales nunca se habían copiado a la copia de Docker**: `crear usuario`,
   `restablecer contraseña` y `cambiar username` estaban **rotas** en la VPS. Copiadas a
   `volumes/functions/` + `docker restart pulse-edge-functions`.

### Auditoría del modal de alta/edición de personal
El usuario pidió revisarlo ("en los campos de texto permite poner números... entre otras cosas").
Se encontraron y arreglaron 10 problemas:
- Nombre/Puesto aceptaban números y símbolos; Teléfono aceptaba letras.
- **Usuario (lo más serio)**: es la credencial real de login. El backend sanea borrando acentos,
  así que `José Pérez` se convertía en `jos.prez` **en silencio** y el empleado recibía un
  usuario que nadie escribió. Ahora se normaliza en el cliente convirtiendo acentos a su letra
  base (`jose.perez`) y **se muestra en vivo el correo de login exacto** que va a quedar.
- Usuario duplicado: la columna tiene `unique`; antes reventaba con error crudo de Postgres.
- **Faltaba el campo Correo**: estaba en el estado y se mandaba vacío, sin input para capturarlo.
- Rol: ofrecía "Administrador" a RH/psicóloga y el servidor rechazaba con 403 recién al guardar.
  Ahora se filtra por rol del que edita, conservando el rol actual como opción para no degradar
  sin querer a un doctor.
- Fecha de ingreso futura y cumpleaños `99-99` pasaban; el modal no cerraba con Escape ni clic fuera.
- Toda la validación vive en `src/utils/validacionUsuario.js` (puro, **+34 tests**).

### Baja de personal: archivar, no borrar (decisión del usuario)
El botón de baja ahora pregunta **Desactivar** (sigue en la lista como inactivo, no entra a la
app) o **Archivar** (desaparece de la lista, no entra, con segunda confirmación). **No hay
borrado**: filtro «Archivados» con contador + botón Restaurar. Restaurar devuelve al empleado
como **Inactivo**, nunca activo — recuperar un error no debería re-otorgar acceso solo.
- Migración **083** `usuarios.archivado` (solo aplicada en la VPS). Archivar implica
  `inactivo = true`, así que el bloqueo de login existente (AuthContext + api/_auth.js) ya lo cubre.
- Hook compartido `hooks/useBajaUsuario.js` para que el flujo sea idéntico en las dos pantallas.
- **Se revirtió el borrado en cascada**: el botón de basura de `/empleados` que se había armado
  antes en esta misma sesión hacía `auth.admin.deleteUser` (irreversible, se llevaba checadas,
  encuestas y fotos de rostro). Contradecía la decisión y estaba desplegado. `eliminarUsuario()`
  y su Edge Function siguen existiendo pero **ya no están cableadas a ningún botón**.

### Verificación
356 tests (34 nuevos), lint sin errores nuevos (los que quedan son la convención preexistente
`'React' is defined but never used`), y **cada cambio probado en vivo** con `claude-in-chrome`
contra `https://mcdentalpulse.duckdns.org` con sesión admin real: ordenamiento, modal, errores
de validación, preview del login, flujo completo de archivar → filtro → restaurar → reactivar
(verificado además contra Postgres: 102 usuarios, ninguno borrado, 0 archivados al cerrar).

### 🚨 Gotcha operativo que hace perder tiempo
